...
from ga_problem import GAProblem

class GAPrimalDecomposition(Algorithm):
    ...
    * Gather :math:`\mu_j^k` from :math:`j \in \mathcal{N}_i` and update
    ...
    # TODO choose ref
    def __init__(self, agent: Agent, initial_condition: np.ndarray, enable_log: bool = False, smart_init = True, ga_seed = 1):
        super(GAPrimalDecomposition, self).__init__(agent, enable_log)
    ...
    def _update_local_solution(self, x: np.ndarray, mu: np.ndarray, mu_neigh: list, stepsize: float, update_runavg, **kwargs):
        ...
        self.x = x
        #self.J = np.asscalar(self.agent.problem.objective_function.eval(x)) #ERROR FOUND
    ...
    def iterate_run(self, stepsize: float, M: float, update_runavg, event: Event, **kwargs):
        ...
        """
        # Build local problem
        # Create a variable z, which is a simple matrix based on the size of the variables used in the problem
        z = Variable(self.x_shape[0] + 1)
        # Create two matrices, A, and A_rho
        # A will be a stack of zeroes and ones based on the size of the input variables
        # They are both transposed to convert them into horizontal vectors
        ...
        #Create rho by matrix multipliying A_rho and the variable
        rho = A_rho @ z
        #Create a set of constraints based on the coupling function and rho
        ...
        #Extend the objective function based on rho
        obj_function = self.objective_function + M * rho
        #Construct a new problem based on the objective function constraints
        pb = GAProblem(obj_function, constraints)

        # solve problem and save data
        if self.x is None:
            self.x = self.local_constraints[0].function.eval(np.zeros((self.x_shape[0] + 1, 1)))
        out = pb.ga_run(self.x_shape, self.x, self.smart_init, ga_seed = self.ga_seed)
